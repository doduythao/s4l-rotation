...
from rot_dataset import LabDataset, RotDataset, FlipDataset
...
parser.add_argument('-vt', '--val_test', default=True, type=str2bool)
...
def _loss(preds, ys, w, is_original):
    L_l = F.cross_entropy(preds[0], ys[0])
    L_u = 0
    for i in range(1, 13):
...
def _train(train_opts):
    ...
    lab_set4 = FlipDataset(lab_set, 4)
    lab_set5 = FlipDataset(lab_set, 5)

    unl_set0 = RotDataset(unl_set, 0)
    unl_set1 = RotDataset(unl_set, 1)
    unl_set2 = RotDataset(unl_set, 2)
    unl_set3 = RotDataset(unl_set, 3)
    unl_set4 = FlipDataset(unl_set, 4)
    unl_set5 = FlipDataset(unl_set, 5)
    ...
    lab_dl_ = DataLoader(lab_set_, batch_size=batch_size, shuffle=False)
    lab_dl0 = DataLoader(lab_set0, batch_size=batch_size, shuffle=False)
    lab_dl1 = DataLoader(lab_set1, batch_size=batch_size, shuffle=False)
    lab_dl2 = DataLoader(lab_set2, batch_size=batch_size, shuffle=False)
    lab_dl3 = DataLoader(lab_set3, batch_size=batch_size, shuffle=False)
    lab_dl4 = DataLoader(lab_set4, batch_size=batch_size, shuffle=False)
    lab_dl5 = DataLoader(lab_set5, batch_size=batch_size, shuffle=False)

    unl_dl0 = DataLoader(unl_set0, batch_size=batch_size * lu_ratio, shuffle=False)
    unl_dl1 = DataLoader(unl_set1, batch_size=batch_size * lu_ratio, shuffle=False)
    unl_dl2 = DataLoader(unl_set2, batch_size=batch_size * lu_ratio, shuffle=False)
    unl_dl3 = DataLoader(unl_set3, batch_size=batch_size * lu_ratio, shuffle=False)
    unl_dl4 = DataLoader(unl_set4, batch_size=batch_size * lu_ratio, shuffle=False)
    unl_dl5 = DataLoader(unl_set5, batch_size=batch_size * lu_ratio, shuffle=False)

    duration = 0.0
    while True:
        for lab_bat, lab0_bat, lab1_bat, lab2_bat, lab3_bat, lab4_bat, lab5_bat, unl0_bat, unl1_bat, unl2_bat, unl3_bat, unl4_bat, unl5_bat in zip(lab_dl_, lab_dl0, lab_dl1, lab_dl2, lab_dl3, lab_dl4, lab_dl5, unl_dl0, unl_dl1, unl_dl2, unl_dl3, unl_dl4, unl_dl5):
            xl, yl = lab_bat
            xo0, yo0 = lab0_bat
            xo1, yo1 = lab1_bat
            xo2, yo2 = lab2_bat
            ...
            _, po4 = model(xo4.cuda())
            _, po5 = model(xo5.cuda())

            _, pu0 = model(xu0.cuda())
            _, pu1 = model(xu1.cuda())
            _, pu2 = model(xu2.cuda())
            _, pu3 = model(xu3.cuda())
            _, pu4 = model(xu4.cuda())
            _, pu5 = model(xu5.cuda())

            loss = _loss([pl, po0, po1, po2, po3, po4, po5, pu0, pu1, pu2, pu3, pu4, pu5],
                         [yl.cuda(), yo0.cuda(), yo1.cuda(), yo2.cuda(), yo3.cuda(), yo4.cuda(), yo5.cuda(), yu0.cuda(), yu1.cuda(), yu2.cuda(), yu3.cuda(), yu4.cuda(), yu5.cuda()],
