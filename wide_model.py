...
#         self.linear = nn.Linear(nStages[3], num_classes)
        self.head_lab = nn.Sequential(nn.Linear(nStages[3], 10))
        self.head_unl = nn.Sequential(nn.Linear(nStages[3], 4))
    ...
    def forward(self, x):
        ...
        lab_logits = self.head_lab(out)
        unl_logits = self.head_unl(out)
        return lab_logits, unl_logits

    def store(self, path_to_dir, step, maximum=5):
        path_to_models = glob.glob(os.path.join(path_to_dir, Wide_ResNet.CHECKPOINT_FILENAME_PATTERN.format('*')))
        if len(path_to_models) == maximum:
            min_step = min([int(os.path.split(path_to_model)[-1][6:-4]) for path_to_model in path_to_models])
            path_to_min_step_model = os.path.join(path_to_dir, Wide_ResNet.CHECKPOINT_FILENAME_PATTERN.format(min_step))
            os.remove(path_to_min_step_model)
        path_to_checkpoint_file = os.path.join(path_to_dir, Wide_ResNet.CHECKPOINT_FILENAME_PATTERN.format(step))
